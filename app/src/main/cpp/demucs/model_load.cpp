...
struct vector_file
{
    ...
    std::size_t position; // Current read position
};

static std::size_t vector_read(void* dest, std::size_t size, std::size_t count, struct vector_file& vec_file) {
    size_t bytes_to_read = size * count;
    if (vec_file.position + bytes_to_read > vec_file.size) {
        bytes_to_read = vec_file.size - vec_file.position;
    }
    std::memcpy(dest, vec_file.data + vec_file.position, bytes_to_read);
    vec_file.position += bytes_to_read;
    return bytes_to_read / size; // Number of elements read
}

static bool vector_eof(struct vector_file& vec_file) {
    return vec_file.position >= vec_file.size;
}
...
// forward declaration
static size_t load_single_tensor1d(struct vector_file &f, std::string &name,
                                   Eigen::Tensor1dXf &matrix, int *ne,
                                   int32_t nelements, bool direct_f32);

static size_t load_single_vector(struct vector_file &f, std::string &name,
                                 Eigen::VectorXf &matrix, int *ne,
                                 int32_t nelements, bool direct_f32);

static size_t load_single_matrix(struct vector_file &f, std::string &name,
                                 Eigen::MatrixXf &matrix, int *ne,
                                 int32_t nelements, bool direct_f32);

static size_t load_single_tensor3d(struct vector_file &f, std::string &name,
                                   Eigen::Tensor3dXf &tensor, int *ne,
                                   int32_t nelements, bool direct_f32);

static size_t load_single_tensor4d(struct vector_file &f, std::string &name,
                                   Eigen::Tensor4dXf &tensor, int *ne,
                                   int32_t nelements, bool direct_f32);

bool demucscpp::load_demucs_model(const std::vector<char>& model_bytes, struct demucs_model* model) {
    // Check if the vector is empty
    if (model_bytes.empty()) {
        return false; // Or handle the error appropriately
    }

    // Call the original function with the data pointer and size
    return load_demucs_model(model_bytes.data(), static_cast<int>(model_bytes.size()), model);
}

// from scripts/convert-pth-to-ggml.py
bool demucscpp::load_demucs_model(const char* model_data, int n_bytes,
                                  struct demucs_model *model)
{
    ...
    // create vector_file struct
    struct vector_file f;
    f.data = model_data;
    f.size = n_bytes;
    f.position = 0;
    ...
    vector_read(&magic, sizeof(uint32_t), 1, f);

    bool direct_f32 = false;

    if (magic == 0x646d6336) // dmc6
    {
        model->use_4source_crosstransformer = false;
        model->num_sources = 6;
    ...
    {
        model->use_4source_crosstransformer = true;
        model->num_sources = 4;
        std::cout << "Model magic is Demucs 4-source" << std::endl;
    }
    else if (magic == 0x646d6337) // custom 2-source
    {
        model->use_4source_crosstransformer = true;
        model->num_sources = 2;
        ...
        // modify a few tensor shapes in the model corresponding to the
        // number of sources
        model->decoder_conv_tr_weight[3] = Eigen::Tensor4dXf(48, 8, 8, 1);
        model->decoder_conv_tr_bias[3] = Eigen::Tensor1dXf(8);

        model->tdecoder_conv_tr_weight[3] = Eigen::Tensor3dXf(48, 4, 8);
        model->tdecoder_conv_tr_bias[3] = Eigen::Tensor1dXf(4);

        direct_f32 = true;

        std::cout << "Model magic is Demucs custom 2-source" << std::endl;
    }
    else
    {
    ...
    model->crosstransformer =
        demucscpp::initialize_crosstransformer(model->use_4source_crosstransformer);
    ...
    for (;;)
    {
        ...
        vector_read(&n_dims, sizeof(int32_t), 1, f);
        vector_read(&length, sizeof(int32_t), 1, f);
        ...
        {
            vector_read(&ne[i], sizeof(int32_t), 1, f);
            nelements *= ne[i];
        }

        std::string name;
        std::vector<char> tmp(length);               // create a buffer
        vector_read(&tmp[0], sizeof(char), tmp.size(), f); // read to buffer
        name.assign(&tmp[0], tmp.size());

        // check if we reached eof of the open file f
        // equivalent in vector<char>
        if (vector_eof(f))
        {
            break;
        }
        ...
        for (int i = 0; i < 4; ++i)
        {
            if (name == "encoder." + std::to_string(i) + ".conv.weight")
            {
                loaded_size = load_single_tensor3d(
                    f, name, model->encoder_conv_weight[i], ne, nelements, direct_f32);
            }
            else if (name == "encoder." + std::to_string(i) + ".conv.bias")
            {
                loaded_size = load_single_tensor1d(
                    f, name, model->encoder_conv_bias[i], ne, nelements, direct_f32);
            }
            else if (name == "encoder." + std::to_string(i) + ".rewrite.weight")
            {
                loaded_size = load_single_tensor3d(
                    f, name, model->encoder_rewrite_weight[i], ne, nelements, direct_f32);
            }
            else if (name == "encoder." + std::to_string(i) + ".rewrite.bias")
            {
                loaded_size = load_single_tensor1d(
                    f, name, model->encoder_rewrite_bias[i], ne, nelements, direct_f32);
            }
        ...
        for (int i = 0; i < 4; ++i)
        {
            if (name == "decoder." + std::to_string(i) + ".conv_tr.weight")
            {
                loaded_size = load_single_tensor4d(
                    f, name, model->decoder_conv_tr_weight[i], ne, nelements, direct_f32);
            }
            else if (name == "decoder." + std::to_string(i) + ".conv_tr.bias")
            {
                loaded_size = load_single_tensor1d(
                    f, name, model->decoder_conv_tr_bias[i], ne, nelements, direct_f32);
            }
            else if (name == "decoder." + std::to_string(i) + ".rewrite.weight")
            {
                loaded_size = load_single_tensor4d(
                    f, name, model->decoder_rewrite_weight[i], ne, nelements, direct_f32);
            }
            else if (name == "decoder." + std::to_string(i) + ".rewrite.bias")
            {
                loaded_size = load_single_tensor1d(
                    f, name, model->decoder_rewrite_bias[i], ne, nelements, direct_f32);
            }
        ...
        for (int i = 0; i < 4; ++i)
        {
            ...
            {
                loaded_size = load_single_tensor3d(
                    f, name, model->tencoder_conv_weight[i], ne, nelements, direct_f32);
            }
            else if (name == "tencoder." + std::to_string(i) + ".conv.bias")
            {
                loaded_size = load_single_tensor1d(
                    f, name, model->tencoder_conv_bias[i], ne, nelements, direct_f32);
            }
            else if (name ==
                     "tencoder." + std::to_string(i) + ".rewrite.weight")
            {
                loaded_size = load_single_tensor3d(
                    f, name, model->tencoder_rewrite_weight[i], ne, nelements, direct_f32);
            }
            else if (name == "tencoder." + std::to_string(i) + ".rewrite.bias")
            {
                loaded_size = load_single_tensor1d(
                    f, name, model->tencoder_rewrite_bias[i], ne, nelements, direct_f32);
            }
        ...
        for (int i = 0; i < 4; ++i)
        {
            ...
            {
                loaded_size = load_single_tensor3d(
                    f, name, model->tdecoder_conv_tr_weight[i], ne, nelements, direct_f32);
            }
            else if (name == "tdecoder." + std::to_string(i) + ".conv_tr.bias")
            {
                loaded_size = load_single_tensor1d(
                    f, name, model->tdecoder_conv_tr_bias[i], ne, nelements, direct_f32);
            }
            else if (name ==
                     "tdecoder." + std::to_string(i) + ".rewrite.weight")
            {
                loaded_size = load_single_tensor3d(
                    f, name, model->tdecoder_rewrite_weight[i], ne, nelements, direct_f32);
            }
            else if (name == "tdecoder." + std::to_string(i) + ".rewrite.bias")
            {
                loaded_size = load_single_tensor1d(
                    f, name, model->tdecoder_rewrite_bias[i], ne, nelements, direct_f32);
            }
        ...
        if (name == "freq_emb.embedding.weight")
        {
            loaded_size = load_single_matrix(
                f, name, model->freq_emb_embedding_weight, ne, nelements, direct_f32);
        }
        else if ((name == "channel_upsampler.weight") && (model->use_4source_crosstransformer))
        {
            auto *ct_4s = static_cast<demucs_crosstransformer_4s *>(
                model->crosstransformer.get());
            loaded_size = load_single_tensor3d(
                f, name, ct_4s->channel_upsampler_weight, ne, nelements, direct_f32);
        }
        else if ((name == "channel_upsampler.bias") && (model->use_4source_crosstransformer))
        {
            auto *ct_4s = static_cast<demucs_crosstransformer_4s *>(
                model->crosstransformer.get());
            loaded_size = load_single_tensor1d(
                f, name, ct_4s->channel_upsampler_bias, ne, nelements, direct_f32);
        }
        else if ((name == "channel_downsampler.weight") && (model->use_4source_crosstransformer))
        {
            auto *ct_4s = static_cast<demucs_crosstransformer_4s *>(
                model->crosstransformer.get());
            loaded_size = load_single_tensor3d(
                f, name, ct_4s->channel_downsampler_weight, ne, nelements, direct_f32);
        }
        else if ((name == "channel_downsampler.bias") && (model->use_4source_crosstransformer))
        {
            auto *ct_4s = static_cast<demucs_crosstransformer_4s *>(
                model->crosstransformer.get());
            loaded_size = load_single_tensor1d(
                f, name, ct_4s->channel_downsampler_bias, ne, nelements, direct_f32);
        }
        else if ((name == "channel_upsampler_t.weight") && (model->use_4source_crosstransformer))
        {
            auto *ct_4s = static_cast<demucs_crosstransformer_4s *>(
                model->crosstransformer.get());
            loaded_size = load_single_tensor3d(
                f, name, ct_4s->channel_upsampler_t_weight, ne, nelements, direct_f32);
        }
        else if ((name == "channel_upsampler_t.bias") && (model->use_4source_crosstransformer))
        {
            auto *ct_4s = static_cast<demucs_crosstransformer_4s *>(
                model->crosstransformer.get());
            loaded_size = load_single_tensor1d(
                f, name, ct_4s->channel_upsampler_t_bias, ne, nelements, direct_f32);
        }
        else if ((name == "channel_downsampler_t.weight") &&
                 (model->use_4source_crosstransformer))
        {
            auto *ct_4s = static_cast<demucs_crosstransformer_4s *>(
                model->crosstransformer.get());
            loaded_size = load_single_tensor3d(
                f, name, ct_4s->channel_downsampler_t_weight, ne, nelements, direct_f32);
        }
        else if ((name == "channel_downsampler_t.bias") && (model->use_4source_crosstransformer))
        {
            auto *ct_4s = static_cast<demucs_crosstransformer_4s *>(
                model->crosstransformer.get());
            loaded_size = load_single_tensor1d(
                f, name, ct_4s->channel_downsampler_t_bias, ne, nelements, direct_f32);
        }
                                 ...
                                 int32_t nelements, bool direct_f32)
{
    ...
    if (!direct_f32) {
        ...
        vector_read(matrix_half.data(), bpe_half, nelements, f);
        ...
        // directly load the float32 matrix
        const size_t bpe_f32 = sizeof(float);
        auto nbytes_tensor = matrix.size() * bpe_f32;

        // create a float Eigen::Matrix with RowMajor order to hold the weights
        Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>
            matrix_row_major =
                Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic,
                            Eigen::RowMajor>::Zero(matrix.rows(), matrix.cols());

        vector_read(matrix_row_major.data(), bpe_f32, nelements, f);

        // copy the weights into the ColMajor matrix
        for (int i = 0; i < ne[0]; i++)
        {
            for (int j = 0; j < ne[1]; j++)
            {
                matrix(i, j) = matrix_row_major(i, j);
            }
        }
                                   ...
                                   int32_t nelements, bool direct_f32)
{
    ...
    if (!direct_f32) {
        ...
        Eigen::Tensor<Eigen::half, 3, Eigen::RowMajor> tensor_half(ne[0], ne[1],
                                                                ne[2]);
        vector_read(tensor_half.data(), bpe_half, nelements, f);
        ...
        // directly load the float32 tensor
        const size_t bpe_f32 = sizeof(float);
        auto nbytes_tensor = tensor.size() * bpe_f32;

        // create a float Eigen::Tensor with RowMajor order to hold the weights
        Eigen::Tensor<float, 3, Eigen::RowMajor> tensor_row_major(ne[0], ne[1], ne[2]);

        vector_read(tensor_row_major.data(), bpe_f32, nelements, f);

        // Manually copy the data from tensor_row_major to tensor
        for (int i = 0; i < ne[0]; ++i)
        {
            for (int j = 0; j < ne[1]; ++j)
            {
                for (int k = 0; k < ne[2]; ++k)
                {
                    tensor(i, j, k) = tensor_row_major(i, j, k);
                }
            }
        }
                                   ...
                                   int32_t nelements, bool direct_f32)
{
    ...
    if (!direct_f32) {
        ...
        // create a float Eigen::Tensor with RowMajor order to hold the weights
        Eigen::Tensor<float, 4, Eigen::RowMajor> tensor_row_major(ne[0], ne[1], ne[2], ne[3]);
        ...
        for (int i = 0; i < ne[0]; ++i)
        {
            for (int j = 0; j < ne[1]; ++j)
            {
                for (int k = 0; k < ne[2]; ++k)
                {
                    for (int l = 0; l < ne[3]; ++l)
                    {
                        tensor(i, j, k, l) = tensor_row_major(i, j, k, l);
                    }
                }
            }
        }
                                   ...
                                   int32_t nelements, bool direct_f32)
{
    ...
    if (!direct_f32) {
        ...
        vector_read(tensor.data(), bpe_f32, nelements, f);
        return nbytes_tensor;
    }
}
                                 ...
                                 int32_t nelements, bool direct_f32)
{
    ...
    if (!direct_f32) {
        ...
        vector_read(vector_half.data(), bpe_half, nelements, f);
        ...
        // directly load the float32 vector
        const size_t bpe_f32 = sizeof(float);
        auto nbytes_vector = vector.size() * bpe_f32;

        vector_read(vector.data(), bpe_f32, nelements, f);
        return nbytes_vector;
    }
}

bool demucscpp_v3::load_demucs_v3_model(const std::vector<char>& model_bytes, struct demucs_v3_model* model) {
    // Check if the vector is empty
    if (model_bytes.empty()) {
        return false; // Or handle the error appropriately
    }
    ...
    // Call the original function with the data pointer and size
    return load_demucs_v3_model(model_bytes.data(), static_cast<int>(model_bytes.size()), model);
}

// from scripts/convert-pth-to-ggml.py
bool demucscpp_v3::load_demucs_v3_model(const char* model_data, int n_bytes,
    ...
    for (;;)
    {
        ...
        for (int encoder_index = 4; encoder_index <= 5; ++encoder_index)
        {
                ...
                "encoder." + std::to_string(encoder_index) + ".conv.weight")
            {
                ...
                {
                    loaded_size = load_single_tensor4d(
                        f, name, model->encoder_4_conv_weight, ne, nelements, direct_f32);
                }
                else
                {
                    loaded_size = load_single_tensor3d(
                        f, name, model->encoder_5_conv_weight, ne, nelements, direct_f32);
                }
            }
            else if (name ==
        ...
        {
            loaded_size = load_single_tensor3d(
                f, name, model->tencoder_4_conv_weight, ne, nelements, direct_f32);
        }
        else if (name == "tencoder.4.conv.bias")
        {
            loaded_size = load_single_tensor1d(
                f, name, model->tencoder_4_conv_bias, ne, nelements, direct_f32);
        }
        ...
        for (int i = 0; i < 2; ++i)
        {
            ...
            {
                loaded_size = load_single_tensor1d(
                    f, name, model->decoder_0_1_conv_tr_bias[i], ne, nelements, direct_f32);
            }
            else if (name == "decoder." + std::to_string(i) + ".norm2.weight")
            {
                loaded_size = load_single_tensor1d(
                    f, name, model->decoder_0_1_norm2_weight[i], ne, nelements, direct_f32);
            }
            else if (name == "decoder." + std::to_string(i) + ".norm2.bias")
            {
                loaded_size = load_single_tensor1d(
                    f, name, model->decoder_0_1_norm2_bias[i], ne, nelements, direct_f32);
            }
            else if (name == "decoder." + std::to_string(i) + ".rewrite.weight")
            {
                if (i == 0)
            ...
            {
                loaded_size = load_single_tensor1d(
                    f, name, model->decoder_0_1_rewrite_bias[i], ne, nelements, direct_f32);
            }
            else if (name == "decoder." + std::to_string(i) + ".norm1.weight")
            {
                loaded_size = load_single_tensor1d(
                    f, name, model->decoder_0_1_norm1_weight[i], ne, nelements, direct_f32);
            }
            else if (name == "decoder." + std::to_string(i) + ".norm1.bias")
            {
                loaded_size = load_single_tensor1d(
                    f, name, model->decoder_0_1_norm1_bias[i], ne, nelements, direct_f32);
            }
        }
        ...
        {
            loaded_size = load_single_tensor3d(
                f, name, model->tdecoder_0_conv_tr_weight, ne, nelements, direct_f32);
        }
        else if (name == "tdecoder.0.conv_tr.bias")
        {
            loaded_size = load_single_tensor1d(
                f, name, model->tdecoder_0_conv_tr_bias, ne, nelements, direct_f32);
        }
        else if (name == "tdecoder.0.norm2.weight")
        {
            loaded_size = load_single_tensor1d(
                f, name, model->tdecoder_0_norm2_weight, ne, nelements, direct_f32);
        }
        else if (name == "tdecoder.0.norm2.bias")
        {
            loaded_size = load_single_tensor1d(
                f, name, model->tdecoder_0_norm2_bias, ne, nelements, direct_f32);
        }
